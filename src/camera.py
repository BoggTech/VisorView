...
class Camera:
    ...
    def get_mouse_distance_from_center(self) -> set:
        ...
        if base.mouseWatcherNode.hasMouse():
            ...
            change_in_x = base.mouseWatcherNode.getMouseX()
            change_in_y = base.mouseWatcherNode.getMouseY()
            ...
            # THIS IS NOT PRECISE. but it works, for now.
    ...
    def enable_relative_mouse(self):
        ...
        if base.mouseWatcherNode.hasMouse():
            base.win.movePointer(0,
                                window_size_x // 2,
                                window_size_y // 2)
        # 0 delay to wait a frame for mouse position to update
        taskMgr.doMethodLater(0, 
                              self.relative_camera_task, 
    ...
    def enable_rotate_control(self):
        ...
        # run on next frame, otherwise mouse pos isnt updated and we get weird jumps
        taskMgr.doMethodLater(0, self.camera_rotate_task, ROTATE_TASK_NAME, priority=DEFAULT_CAMERA_TASK_PRIORITY)
    ...
    # Function called when mouse3 is held down and zooming is active.
    def enable_zoom_control(self):
        ...
        self.enable_relative_mouse()
        # run on next frame, otherwise mouse pos isnt updated and we get weird jumps
        taskMgr.doMethodLater(0, self.camera_zoom_task, ZOOM_TASK_NAME, priority=DEFAULT_CAMERA_TASK_PRIORITY)
    ...
    # Function called when mouse2 is held down and zooming is active.
    def enable_pan_control(self):
        ...
        self.enable_relative_mouse()
        # run on next frame, otherwise mouse pos isnt updated and we get weird jumps
        taskMgr.doMethodLater(0, self.camera_pan_task, PAN_TASK_NAME, priority=DEFAULT_CAMERA_TASK_PRIORITY)
