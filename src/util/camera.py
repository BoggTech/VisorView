...
class Camera(NodePath):
    ...
    def __init__(self) -> None:
        """Disables the default camera controls, moves the camera to its default position, reparents the camera, and
        enables mouse input.
        """
    ...
    def get_mouse_distance_from_center(self):
        """Function that gets the mouse's distance from the center, represented as as a float in the range (-1, 1). For
        odd window sizes, the center pixel is rounded down.

        :rtype: tuple
        :return: A tuple S where S[0] is x and S[1] is y
        """
    ...
    def relative_camera_task(self, task):
        """Task that resets the camera to the center. It should be run with a higher (asin, the actual number) priority
        so that it runs last in the chain.
        """
        self.move_mouse_to_center()
        return Task.cont
